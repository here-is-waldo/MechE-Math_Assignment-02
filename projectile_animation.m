...
function projectile_animation()
    theta_guess = 0; % radians
    t_guess = 1; % seconds

    % solve
    [theta_i, t_coll] = solve_init_conditions(theta_guess, t_guess);

    % run simulation 
    run_simulation(theta_i, t_coll)
...
function [theta_i, t_coll] = solve_init_conditions(theta_guess, t_guess)
    ...
    % input vector: Vin = [theta; t]
    % trying to zero the output which is pos_diff = [x_diff, y_diff]
    dist_fcn = @(Vin) projectile_traj(Vin(1), Vin(2)) - target_traj(Vin(2));
